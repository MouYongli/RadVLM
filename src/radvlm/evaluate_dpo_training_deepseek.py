...
def evaluate_pre_training():
    """Evaluate the DPO-trained DeepSeek VL2 model from file on a validate set"""
    
    print("Evaluating DPO-trained DeepSeek VL2 model...", flush=True)
    
    here = os.path.dirname(os.path.abspath(__file__))
    model_path = os.path.abspath(os.path.join(here, "../../results/dpo/deepseek-vl2-mimic-cxr-dpo-lora-r16-lr5e-5-beta0.1-model32-5pctdataset-frommodel23-1e-2lambda-discrete-processed-corrected-sampling/best_model"))
    ...
    output_file = '/pfss/mlde/workspaces/mlde_wsp_RWTH_MedReport/ag88juba/RadVLM/results/generated_reports/generated_reports_dpo-lora-r16-lr5e-5-beta0.1-model32-5pctdataset-frommodel23-1e-2lambda-discrete-processed-corrected-sampling.txt'
