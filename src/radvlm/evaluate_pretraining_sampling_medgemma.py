...
def evaluate_pre_training():
    ...
    model_path = os.path.abspath(os.path.join(here, "../../results/pretraining/medgemma-1.5-mimic-cxr-poc-lora-r64-lr1e-4-3epochs-linear-5pctwarmup-6earlystop-100pct-final"))
    evaluator = MedGemmaPretrainingEvaluator(model_path=model_path, base_model_path=MEDGEMMA_BASE_MODEL_PATH, strategy="sampling", temperature=0.7, top_p=0.9)
    ...
    output_file = '/pfss/mlde/workspaces/mlde_wsp_RWTH_MedReport/ag88juba/RadVLM/results/generated_reports/generated_reports_sampling_medgemma_poc-lora-r64-lr1e-4-3epochs-linear-5pctwarmup-6earlystop-100pct-final.txt'
